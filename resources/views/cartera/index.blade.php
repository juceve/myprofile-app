    ...
    <div class="space-y-6">
        ...
        <x-card class="p-5">
            <form method="GET" action="{{ route('cartera.index') }}" class="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
                ...
                <select name="situacion" class="rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none focus:border-emerald-400 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 xl:col-span-2"><option value="vigente" @selected(request('situacion', 'vigente') === 'vigente')>Cartera vigente</option><option value="cancelada_por_mandante" @selected(request('situacion') === 'cancelada_por_mandante')>Saldadas por mandante</option></select>
                <div class="flex gap-2 xl:col-span-3"><select name="estado" class="min-w-0 flex-1 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none focus:border-emerald-400 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"><option value="">Estado origen</option>@foreach($estados as $estado)<option value="{{ $estado }}" @selected(request('estado') === $estado)>{{ $estado }}</option>@endforeach</select><x-primary-button>Filtrar</x-primary-button></div>
            </form>
        </x-card>
                ...
                <form method="POST" action="{{ route('cartera.importar') }}" enctype="multipart/form-data" data-loading-form data-progress-form class="p-6">
                    ...
                    <div data-import-progress class="mt-5 hidden rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/60" aria-live="polite">
                        <div class="flex items-center justify-between gap-3 text-xs font-semibold">
                            <span class="flex items-center gap-2 text-slate-600 dark:text-slate-300"><span data-import-spinner class="hidden h-3.5 w-3.5 animate-spin rounded-full border-2 border-slate-300 border-t-emerald-600 dark:border-slate-600 dark:border-t-emerald-300" aria-hidden="true"></span><span data-import-progress-message>Preparando archivo...</span></span>
                ...
                @else
                    <div class="mt-5 max-h-[60vh] overflow-auto">
                        <table class="w-full min-w-[760px] text-left text-sm">
                            <thead class="text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400"><tr><th class="py-3 pr-4">Archivo</th><th class="py-3 pr-4">Estado</th><th class="py-3 pr-4">Filas procesadas</th><th class="py-3 pr-4">Nuevas</th><th class="py-3 pr-4">Actualizadas</th><th class="py-3 pr-4">Canceladas por mandante</th><th class="py-3 pr-4">Reingresadas</th><th class="py-3 pr-4">Procesado</th></tr></thead>
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
